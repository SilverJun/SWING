...
그리고 최대한 객체지향으로 설계하지만 코드가 산으로 가는 상속관계, 스파게티 코드가 되고말죠.
항상 계획과 생각한 대로 구현이 쉽지만은 않았습니다.

그럴때 마다 몇가지 교훈을 얻게되죠.

저는 최적화와 버그를 해결하기 위한 컴퓨터 저수준의 이해가 필요하다고 느꼈고
더 나은 구조를 위해 여러가지 객체지향 패턴들을 공부하려고 했습니다.
...
멀티패러다임 언어로 다양하게 구현이 가능합니다. 또한 오픈소스로 전환하면서 소스를 분석할 수 있게 되었습니다.

저는 iOS프로젝트를 스위프트로 직접 개발하면서 스위프트를 배웠습니다.

스위프트로 애플제품의 어플리케이션을 개발하는 것 보다 애플이 제시한 스위프트 언어 그 자체가 더 흥미롭게 다가왔습니다.
...
let 변수명:타입      //불가능! 상수는 선언과 동시에 값을 부여해야 함수나
```
### 컬렉션 타입 선언
//TODO
